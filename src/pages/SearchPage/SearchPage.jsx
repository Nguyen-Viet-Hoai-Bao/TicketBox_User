...
const SearchPage = () => {
    ...
    console.log("station: Return? " + station.returnAt + " --- StartAt? " + station.startAt+ " --- arrivalAt? " + station.arrivalAt+ " --- departurePoint? "  + station.start + "--- arrivalPoint" + station.arrival);
    ...
    useEffect(() => {
        ...
        // Sử dụng hàm callback để đảm bảo rằng departurePoint và arrivalPoint đã được cập nhật
    ...
    const handleStation = (id) => {
        const url = `http://localhost:8080/api/v1/stations?name=${id}`;
        console.log("handleStation: "+url);
        fetch(url, {
            method: "GET",
        })
